...
{
    public class Transport : IService
    {
        ...
        public Transport(string serviceName, string city, string country)
        {
        ...
        public int Add(IService service)
        {
            using (SQLiteConnection conn = new SQLiteConnection(connection))
            {
                try
                {
                    ...
                    {
                        string sql = "INSERT INTO Services (SERVICENAME, SERVICETYPE, CITY, COUNTRY) VALUES('" + service.serviceName + "', 'Transport"
        ...
        public int Edit(int serviceID, IService service)
        {
            using (SQLiteConnection conn = new SQLiteConnection(connection))
            {
                try
                {
                    ...
                    if (conn.State == System.Data.ConnectionState.Open)
                    {
                        string sql = "UPDATE Services SET SERVICENAME ='" + service.serviceName + "', SERVICETYPE ='Transport', CITY ='"
